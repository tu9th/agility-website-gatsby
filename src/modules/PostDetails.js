...
const PostDetails = ({ item, dynamicPageItem, page }) => {
  ...
  const hasTweets =
    post.textblob && post.textblob.indexOf('class="twitter-tweet"') !== -1
    ...
    <section className="blog-post-details">
      <div className="">
        <div className="container p-w-small rich-text">
          ...
          {post.subTitle && <h4 className="h4">{post.subTitle}</h4>}
          <PostTags post={post} />
            ...
            <div className="author">
              <div className="author-image">
                <img
                  src={
                    author.image
                      ? author.image.url + "?w=100"
                      : "https://static.agilitycms.com/authors/blank-head-profile-pic.jpg?w=100"
                  }
                  alt={author.title ? author.title : "image author"}
                />
              </div>
          ...
          {item.backButton && item.backButton.text && item.backButton.href && (
            <Link to={item.backButton.href} className="back d-flex ai-center">
              <img
